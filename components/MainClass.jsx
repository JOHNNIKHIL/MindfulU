  ...
  return (
    <div>
      <div className="bg-violet-600 mix-blend-colors pb-5">
        ...
        <div className="text-white ease-in font-small font-small p-4 m-3 grid grid-cols-1 md:grid-cols-2 rounded-2xl mb-5 bg-gradient-to-br from-teal-600 shadow-2xl z-50">
          <div className="p-4">
        ...
        <div className="box-content pt-3 p-3 mt-3 mb-3 z-50 shadow-2xl bg-gradient-to-br from-blue-600 to-[#706bf4]">
          ...
          <div className="grid grid-cols-1 mx-4 md:grid-cols-2 lg:grid-cols-3">
            {currentBlogs.map((blog) => (
              <CSSTransition key={blog.id} in={true} appear={true} timeout={500} classNames="fade">
              <div>{blogComponents[blog.id]}</div>
            </CSSTransition>
            ))}
          </div>
        </div>
